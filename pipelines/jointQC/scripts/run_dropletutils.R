...
plot(br.out$rank, br.out$total, log = "xy", xlab = "Rank", ylab = "Total", main = opts$library)
o <- order(br.out$rank)
lines(br.out$rank[o], br.out$fitted[o], col = "red")
abline(h = knee, col = "dodgerblue", lty = 2)
abline(h = inflection, col = "forestgreen", lty = 2)
legend("bottomleft", lty = 2, col = c("dodgerblue", "forestgreen"), legend = c(paste0("knee (", knee, ")"), paste0("inflection (", inflection, ")")))
dev.off()
...
plot(e.out$Total, -e.out$LogProb, col = ifelse(is.cell, "red", "black"),
  xlab = "Total UMI count", ylab = "-Log Probability", main = opts$library)
dev.off()
